...
public class Laboratory {
	...
	public void fillLab(Date initialDate, Date endDate, int time) throws IOException, ParseException, InterruptedException{
		...
		for(ExecutionThread execution: executions){
			execution.setConfig(time, initialDate, endDate);
			execution.start();
		}
		for(ExecutionThread execution: executions)execution.join();
		evaluateLab(initialDate, endDate, time);
		Date end = new Date();
		System.out.println("END: " +end);
		System.out.println("Total -> "+(end.getTime()-start.getTime()));
	}
	
	public void evaluateLab(Date initialDate, Date endDate, int time) throws ParseException, IOException{
		...
		while (tempDate.before(endDate)){
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter(new File("results/range_"+time+"/"+initDate+"/Waira.csv")));
